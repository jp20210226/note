...
public class Aria 
{
    ...
    private static string ToJson(JsonClass json)
    {
        ...
        for (int i = 0; i < json._params.Count; i++)
        {
            ...
            else if (json._method == "aria2.tellStatus")
            {
                ...
                {
                    if (json._params.Count > 2)//有指定返回参数
                    {
                        str += "[\"" + json._params[i] + "\"";
                    }
                    else
    ...
    /// <summary>
    /// 开启Aria2，如果Aria2进程存在则跳过,路径 Application.streamingAssetsPath + /plugins/aria2/aria2c.exe
    /// </summary>
    public void Aria2Start()
    {
        Process[] Processes = Process.GetProcesses();
        foreach (Process p in Processes)
        {
            if (p.ProcessName.Contains("aria2c"))
            {
                return;
            }
        }
        ProcessStartInfo psi = new ProcessStartInfo();
        psi.WorkingDirectory = Application.streamingAssetsPath + @"/plugins/aria2";
        psi.FileName = Application.streamingAssetsPath + @"/plugins/aria2/aria2c.exe";
        psi.Arguments = "  --conf-path=aria2.conf";
